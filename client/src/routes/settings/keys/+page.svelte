<script>
  ...
  import { getKeys, clearAllData, storeKeys } from '$lib/crypto/index.js';
  ...
  const BIP39_WORDLIST = [
    ...
    'embody','embrace','emerge','emotion','employ','empty','enable','enact','end','endless',
    ...
    'flee','flight','flip','float','flock','floor','flower','fluid','flush','flute',
    ...
    'gown','grab','grace','grade','gradual','grain','grand','grape','grass','gravity',
    ...
    'hockey','hold','hole','holiday','hollow','holy','home','honey','hood','hope',
    ...
    'insect','inside','insight','insist','inspire','install','intact','interest','into','invest',
    ...
    'little','live','lizard','load','loan','lobster','local','location','lock','locust',
    'lodge','log','lonely','long','loop','lottery','loud','lounge','love','loyal',
    ...
    'marsh','mask','mass','master','match','material','math','matrix','matter','maximum',
    'maze','meadow','mean','measure','meat','mechanic','medal','media','melody','melt',
    'member','memory','mention','menu','mercy','merge','merit','merry','mesh','message',
    'metal','method','middle','midnight','milk','million','mimic','mind','minimum','minister',
    ...
    'net','network','neutral','never','news','next','nice','niche','niece','night',
    'nine','noble','noise','nominee','noodle','normal','north','nose','notable','note',
    ...
    'off','offer','office','officer','official','often','oil','okay','old','olive',
    ...
    'shadow','shaft','shallow','share','shark','sharp','shave','shed','shell','sheriff',
    ...
    'switch','sword','symbol','symptom','syrup','system','table','tablet','tackle','tag',
  ...
  function pubKeyToFingerprint(pubKey) {
    ...
    const words = [];
    for (let i = 0; i < 4; i++) {
      const idx = ((hash[i * 2] << 8) | hash[i * 2 + 1]) % BIP39_WORDLIST.length;
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (keys.identityPrivateKey && keys.signingPrivateKey) {
        ...
        identityPubKey = x25519.getPublicKey(keys.identityPrivateKey);
        signingPubKey = ed25519.getPublicKey(keys.signingPrivateKey);
        fingerprint = pubKeyToFingerprint(identityPubKey);
      }
    } catch (err) {
  ...
  async function exportKeys() {
    const keys = await getKeys();
    if (!keys.identityPrivateKey || !keys.signingPrivateKey) return;

    const combined = new Uint8Array(64);
    combined.set(keys.identityPrivateKey, 0);
    combined.set(keys.signingPrivateKey, 32);

    const blob = new Blob([combined], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'cypherchat-keys.bin';
    a.click();
  ...
  async function importKeys(event) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        await storeKeys(data.slice(0, 32), data.slice(32, 64), null);
        const { x25519 } = await import('@noble/curves/ed25519.js');
        const { ed25519 } = await import('@noble/curves/ed25519.js');
        identityPubKey = x25519.getPublicKey(data.slice(0, 32));
        signingPubKey = ed25519.getPublicKey(data.slice(32, 64));
        fingerprint = pubKeyToFingerprint(identityPubKey);
        error = '';
      } catch (err) {
...
<div class="container">
  <h1 class="page-title">Your Keys</h1>
  <p class="page-subtitle">Manage your cryptographic identity</p>

  <div class="card">
    {#if fingerprint}
      <div style="text-align:center; margin-bottom:1.5rem;">
        <p style="color:var(--text-muted); font-size:0.875rem; margin-bottom:0.5rem;">Identity Fingerprint</p>
        <div class="key-fingerprint">{fingerprint}</div>
      </div>
    {/if}
    ...
    <div style="display:flex; flex-direction:column; gap:0.75rem;">
      <button class="btn btn-secondary" on:click={exportKeys}>
        Export Encrypted Key File
      </button>

      <label class="btn btn-secondary" style="cursor:pointer;">
        Import Key File
        <input type="file" accept=".bin" on:change={importKeys} style="display:none;" />
      </label>

      <a href="/settings/2fa" class="btn btn-secondary" style="text-align:center;">
        Setup 2FA
      </a>

      <a href="/settings/sessions" class="btn btn-secondary" style="text-align:center;">
        Manage Sessions
      </a>

      <a href="/chat" class="btn btn-secondary" style="text-align:center;">
        Open Chat
      </a>
    ...
    {#if showDeleteConfirm}
      <div style="margin-top:1.5rem; padding:1rem; background:var(--bg); border-radius:var(--radius); border:1px solid var(--danger);">
        <p style="color:var(--danger); font-weight:600; margin-bottom:0.5rem;">Are you sure?</p>
        <p style="font-size:0.875rem; color:var(--text-muted); margin-bottom:1rem;">
          This will permanently delete your account and all data. This cannot be undone.
        </p>
        <div style="display:flex; gap:0.5rem;">
          <button class="btn btn-danger" on:click={deleteAccount} style="flex:1;">Yes, Delete</button>
          <button class="btn btn-secondary" on:click={() => showDeleteConfirm = false} style="flex:1;">Cancel</button>
        </div>
      </div>
    {/if}
  </div>
