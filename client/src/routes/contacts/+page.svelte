<script>
  ...
  function getFingerprintFromB64(pubKeyB64) {
    try {
      const pubKey = Uint8Array.from(atob(pubKeyB64), c => c.charCodeAt(0));
      return pubKeyToFingerprint(pubKey);
    } catch {
      return 'unknown';
    }
  }
  ...
  async function handleAddContact() {
    ...
    loading = true;
    try {
      const contact = await contacts.addContact(username, alias);
      success = `Added ${contact.alias || contact.username} (${getFingerprintFromB64(contact.publicKey)})`;
      username = '';
      alias = '';
    } catch (err) {
      error = err.message;
...
<div class="container">
  ...
  <p class="page-subtitle">Find someone by username</p>

  <div class="card">
    <div class="form-group">
      <label for="username">Username</label>
      <input id="username" type="text" bind:value={username} placeholder="friend_username" />
    </div>

    <div class="form-group">
      <label for="alias">Alias (optional)</label>
      <input id="alias" type="text" bind:value={alias} placeholder="Best Friend" />
    </div>

    {#if error}<p class="error">{error}</p>{/if}
    {#if success}<p class="success">{success}</p>{/if}

    <button class="btn" on:click={handleAddContact} disabled={loading || !username} style="width:100%;">
  ...
  {#if $contacts.length > 0}
    <div class="card" style="margin-top:1.5rem;">
      <h3 style="margin-bottom:1rem;">Your Contacts</h3>
      {#each $contacts as contact}
        <div class="session-item">
          <div>
            <div style="font-weight:600;">{contact.alias || contact.username}</div>
            <div style="font-size:0.8rem; color:var(--text-muted); font-family:monospace;">
              {getFingerprintFromB64(contact.publicKey)}
            </div>
          </div>
        </div>
      {/each}
  ...
  <div style="text-align:center; margin-top:1rem;">
    <a href="/chat" style="color:var(--text-muted);">← Back to Chat</a>
  </div>
</div>
